...
class Login extends Base
{
    ...
    public function load()
    {
        ...
        $this->registerEvent('sasl2success', 'onStart');
    ...
    public function display()
    {
        ...
        $this->view->assign('domain', !empty($configuration->xmppdomain) ? $configuration->xmppdomain : 'movim.eu');
        $this->view->assign('invitation', null);

        if ($this->get('i') && Validator::length(8)->isValid($this->get('i'))) {
        ...
        $this->view->assign('oauthAutoLogin', config('oauth.enabled') && config('oauth.auto_login', true) && $this->trustedIdentity !== null);

        if (isset($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW'], $_SERVER['HTTP_EMAIL'])
            && Validator::email()->length(6, 40)->isValid($_SERVER['HTTP_EMAIL'])) {
            [, $host] = explode('@', $_SERVER['HTTP_EMAIL']);
    ...
    public function prepareError(string $error = 'default', ?string $errorMessage = null)
    {
        ...
        $view->assign('error', $error_text == $key ? $this->__('error.default') : $error_text);
        $view->assign('errormessage', $errorMessage);
        return $view->draw('_login_error');
    }

    public function onFailAuth() { $this->showErrorBlock('fail_auth'); }
    public function onDNSError() { $this->showErrorBlock('dns'); }
    ...
    {
        $error = match ($packet->content) {
            'invalid-mechanism', 'malformed-request' => 'mechanism',
            'not-authorized', 'bad-auth' => 'wrong_account',
            default => 'fail_auth',
        };
    ...
    public function ajaxLogin($form, string $sessionId, string $timezone)
    {
        ...
        $this->doLogin(strtolower($form->username->value), $form->password->value, $timezone, $sessionId);
    }
    ...
    public function ajaxHttpOAuthLogin(string $sessionId, string $timezone)
    {
        ...
        if (!empty($configuration->xmppwhitelist) && !in_array($host, $configuration->xmppwhitelist)) {
        ...
        if ($stale = \App\Session::find($sessionId)) $stale->delete();

        try {
            $tokenResponse = (new TokenIssuer)->issueToken($trustedIdentity['raw_identity'], $login);
        } catch (\Throwable $e) {
            logError($e);
        ...
        $session->init(username: $username, password: generateKey(64), host: $host, sessionId: $sessionId, timezone: $timezone);
        $session->save();

        $payload = json_encode(['func' => 'message', 'b' => ['w' => 'Login', 'f' => 'ajaxOAuthDaemonLogin', 'p' => [$login, $tokenResponse['access_token'], $timezone]]]);
        $dispatched = requestAPI('ajax', post: ['sid' => $sessionId, 'json' => rawurlencode($payload)]);
        if ($dispatched === false) {
            $session->delete();
    ...
    public function ajaxQuickLogin(string $deviceId, string $login, string $key, string $timezone, ?string $sessionId = null, ?bool $check = false)
    {
        ...
        try {
            ...
            if ($user) {
                ...
                if ($ciphertext) {
                    ...
                    $this->doLogin(login: $login, password: $password, sessionId: $sessionId, timezone: $timezone, deviceId: $deviceId);
                } else {
                    $this->rpc('Login.clearQuick');
                }
            } else {
    ...
    private function doLogin(string $login, string $password, string $timezone, ?string $sessionId = null, ?string $deviceId = null)
    {
        $configuration = Configuration::get();
        if (!validateJid($login)) {
            $this->showErrorBlock('login_format');
        ...
        $s = new \App\Session;
        $s->init(username: $username, password: $password, host: $host, sessionId: $sessionId, timezone: $timezone);
        $s->save();
        $linker = linker($s->id);
        if ($linker == null) {
            $s->delete();
    ...
    private function queueXmppConnection(string $login, string $host): void
    {
        ...
        $linker->session->set('pending_stream_init', ['host' => $host, 'login' => $login]);
        $this->flushPendingStreamInit();
    }
    ...
    {
        $linker = linker($this->sessionId);
        if ($linker == null || !$linker->connected()) return;
        $pending = $linker->session->get('pending_stream_init');
        if (!is_array($pending) || empty($pending['host']) || empty($pending['login'])) return;
        $linker->writeXMPP(Stream::init(to: $pending['host'], from: $pending['login']));
